...
class GoodsSearchPagingSource(private val repository: MallRepository, private val keyword: String) : PagingSource<Int, Goods>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Goods> {
        val page = params.key ?: 1
        return try {
            val response = repository.searchGoods(keyword, page, params.loadSize)
            if (response.success) {
